...
class MailNode:
	def __init__(self):
		self.header = None
		self.subject = None
		self.mailTimeStamp = None
		self.noOfAttachments = 0
...
def readOneFile(filePath, outputFolderStr):
	# f = open('../enron_data/maildir/lay-k/enron/3.', 'r')
	f = open(filePath, 'r')
	fout = open(outputFolderStr+'/'+os.path.basename(f.name),'w')
	for line in f:
		if(dropLine(line)):
			pass
		else:
			# print line
			replacedLine = replaceUnwantedAnywhere(line)
			replacedLine = replaceUnwantedAtEnd(replacedLine)
			writeOneLine(fout, replacedLine )

	fout.close()
	f.close()

def browseFolder(inputFolderStr, outputFolderStr):
	fileList = glob.glob(inputFolderStr+'/**/all_documents/*')
	for fStr in fileList:
		readOneFile(fStr, outputFolderStr)
...
parser = EnronParser()
# parser.start('../data/raw/maildir/skilling-j', '../output/firstpass/maildir/skilling-j')
parser.start('../data/raw/maildir/delainey-d', '../output/secondpass/maildir/delainey-d')

# parser.startOneFile('/home/mprakash/Documents/enron-data/maildir/lay-k/inbox','125.','output/participants-1')
# parser.startOneFile('../enron-data/maildir/skilling-j/inbox','1519.','runtime')
